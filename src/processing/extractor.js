...
/**
 * Strip trailing punctuation from a URL that's likely sentence-level, not part of the URL.
 * @param {string} url - Raw extracted URL
...
export function extractIOCs(events) {
  ...
  for (const event of events) {
    ...
    // Extract URLs first — we'll use these to deduplicate domains
    const rawUrls = text.match(urlRegex) || [];
    const extractedUrls = [];
    const urlHostnames = new Set();
    ...
    for (const rawUrl of rawUrls) {
      ...
      try {
        ...
        urlHostnames.add(parsed.hostname.toLowerCase());
      } catch {
        // Invalid URL, skip
      }
    }

    // Extract domains — skip if domain is a hostname inside an already-extracted URL
