...
import { fetchThreatFeed, fetchRSSFeed, getDemoData } from "./ingestion/feeds.js";
...
import { correlateIOCs } from "./processing/correlator.js";
import { enrichIOC } from "./enrichment/enrich.js";
import { batchAnalyzeThreats } from "./ai/analyzer.js";
...
export async function executePipeline(feeds, options = {}) {
  ...
  for (const feed of feeds) {
    try {
      ...
      } else if (feed.type === "static") {
        rawData = await getDemoData();
      } else if (feed.type === "stix" || feed.type === "file") {
        logger.warn({ feed: feed.name, type: feed.type }, "Feed type not yet fully implemented, attempting REST fetch");
