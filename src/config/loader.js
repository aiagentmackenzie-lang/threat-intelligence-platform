...
const FeedSchema = z.object({
  name: z.string().min(1),
  type: z.enum(["rest", "rss", "stix", "file", "static"]),
...
// Feed types that are not yet fully implemented
const UNSUPPORTED_FEED_TYPES = new Set(["stix", "file"]);
...
export async function loadConfig(configPath = "./config/feeds.json") {
  try {
    ...
    for (const feed of result.data) {
      ...
      if (UNSUPPORTED_FEED_TYPES.has(feed.type) && feed.enabled) {
        logger.warn({ feed: feed.name, type: feed.type }, "Feed type is not yet fully implemented; it may not work as expected");
      }
    }
