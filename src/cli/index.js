...
async function main() {
  try {
    ...
    const validFormats = ["console", "json", "ndjson", "stix"];
    if (args.format && !validFormats.includes(args.format)) {
      console.error(`Invalid format: ${args.format}. Valid formats: ${validFormats.join(", ")}`);
      process.exit(1);
    }
    ...
    if (args.feeds) {
      ...
      const before = enabledFeeds.length;
      enabledFeeds = enabledFeeds.filter((f) => feedNames.includes(f.name));
      const matched = enabledFeeds.length;
      if (matched === 0) {
        console.error(`Warning: --feeds filter "${args.feeds}" matched 0 feeds. Available: ${feeds.filter(f => f.enabled).map(f => f.name).join(", ") || "none"}`);
        process.exit(1);
      } else if (matched < before) {
        console.log(`Note: --feeds filter selected ${matched} of ${before} enabled feed(s)`);
      }
    }
